...
5. `.aitools/channel/session-uyZ7TELqpP/20260316T190000Z_s3_running-estimate.json` -- S3 running estimate from session uyZ7TELqpP. NOTE: lives at session-scoped path; should migrate to `.aitools/channel/running-estimate.json` per decision #50 when channel infrastructure is built.
6. `harvesting/2026-03-16_carry-forward-design.md` -- running estimate schema, lifecycle, integration with decisions #4/#22/#26/#44
7. `plans/mission-command-briefing/delegation-evolution.md` -- how the execution protocol was born (7 user interventions)
8. `harvesting/2026-03-16_aar-tool-ops-plan.md` -- AAR from tool-ops plan execution (read first 100 lines minimum for executive summary)
9. `harvesting/2026-03-16_investigate-estimate-enforcement.md` -- barrier analysis on running estimate enforcement (Option 4 selected)
10. `harvesting/2026-03-16_briefing-analysis.md` -- S2 quality audit of the brief
...
### D3. Ten approved decisions (not yet implemented)
...
3. **Fix OT-2: replace `.gitignore` blanket** -- replace `.aitools/` in root `.gitignore` with selective patterns (`.aitools/scratch/`, `.aitools/channel/session-*/`, `.aitools/.current-session`, `.aitools/channel/.current-session`). This unblocks namespace consolidation, briefings tracking, and running-estimate tracking.
...
7. **Recency heuristic provenance research** (OT-5). Only continue if it directly supports plan writing.
8. **Observe-mode promotion review** (R6, 1003 log entries). Analyze only if hook enforcement changes are part of a plan batch.
...
| # | Thread | Next action |
|---|--------|-------------|
| OT-2 | `.gitignore` blocker for `.aitools/` consolidation | Replace blanket `.aitools/` with selective patterns. Wave 1 item 3 |
...
| OT-5 | Recency heuristic provenance | Continue web research. Soft exclusion 7 |
...
- **Alternative**: Pre-approve Write by touching a placeholder file in the target directory before launching the subagent
...
The plan-writing protocol and Plan Writer delegation template are defined in the prior handoff prompt at `plans/mission-command-briefing/handoff-prompt.md`, sections E and F. They remain valid and unchanged. Key points:
