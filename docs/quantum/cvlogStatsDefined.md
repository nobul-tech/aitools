...
| 7.  | FSM writes buf                   | Number of synchronous writes to metadata                                                |
| 8.  | FSM writes abm                   | Number of writes to the allocation bit map                                              |
| 9.  | FSM writes inode                 | Number of writes of inodes                                                              |
| 10. | FSM writes ganged                | Number of inode writes that were able to be concatenated to<br>improve efficiency       |
...
| 13. | FSM journal bytes used avg       | Average number of bytes in the journal                                                  |
| 14. | FSM journal bytes used max       | Maximum number of bytes in the journal                                                  |
...
| 17. | FSM wait revokes                 | Number of times the FSM had to wait to switch a file from<br>exclusive to shared access |
...
| 21. | FSM threads max busy hi-prio     | Maximum number of threads working on operations except<br>open and lookup               |
| 22. | FSM threads max busy lo-prio     | Maximum number of threads working on open and lookup                                    |
| 23. | FSM threads max busy dmig        | Maximum number of threads working on data migration                                     |
...
| 27. | FSM msg queue hi-prio max        | Largest size of the high priority queue                                                 |
...
| 30. | FSM msg queue lo-prio max        | Largest size of the low priority queue                                                  |
...
| 34. | FSM msg queue events now         | Current size of the event queue                                                         |
| 35. | FSM msg queue events min         | Smallest size of the event queue                                                        |
| 36. | FSM msg queue events max         | Largest size of the event queue                                                         |
...
| 40. | FSM cache free incore inodes now | Current number of free in-core inodes                                                   |
...
|     |                                 | cache                                                            |
| 45. | FSM cache buffer hits %         | Percentage of metadata entries that were found in the FSM's      |
|     |                                 | buffer cache                                                     |
...
| 52. | FSM AllocSession sessions       | Number of times clients allocated from their reserved space.     |
...
| 56. | FSM extent lookups              | Number of times the requested data block was found               |
...
| 65. | Getattr cnt/avg/min/max         | Get the attributes of a file or directory                        |
| 66. | Readdir cnt/avg/min/max         | Read a directory                                                 |
| 67. | Setattr cnt/avg/min/max         | Set the attributes of a file or directory                        |
...
| 84.  | ExpandInodes cnt/avg/min/max     | Expand the pool of available inodes                                  |
| 85.  | AllocSpaceApi cnt/avg/min/max    | Grow the size of a file                                              |
| 86.  | GetNTSecurity cnt/avg/min/max    | Returns the NT security descriptor associated with a file or         |
|      |                                  | directory                                                            |
| 87.  | SetNTSecurity cnt/avg/min/max    | Sets the NT security descriptor for a file or directory              |
...
| 90.  | SwapExtents cnt/avg/min/max      | Swap the allocation of one file with that of another                 |
...
| 97.  | OpenRet cnt/avg/min/max          | Client is switching a file to a shared access state                  |
...
| 104. | VopCapNegotiate cnt/avg/min/max  | Return the capabilities of the FSM running the file system           |
...
| 109. | VopInsertBlobTag cnt/avg/min/max | Used when de-duplicating a file                                      |
| 110. | VopSubTypeXattr cnt/avg/min/max  | Used when doing data migration                                       |
| 111. | BulkGetattr cnt/avg/min/max      | Get the attributes of multiple files or directories                  |
| 112. | GetResyncAttr cnt/avg/min/max    | Resynchronize inode attributes with a client                         |
| 113. | NotifyContMod cnt/avg/min/max    | Xsan Spotlight notification that the contents of a file changed      |
| 114. | GetLinkInfo cnt/avg/min/max      | Process a reverse path lookup element in the path of the file        |
...
|      |                                  | request                                                              |
| 117. | InodeCacheCheck cnt/avg/min/max  | 3.5-only; debugging hook, not used on production systems             |
...
| 121. | PartRequest cnt/avg/min/max      | Return stripe group information to a client                          |
...
| 130. | InodeChange cnt/avg/min/max      | Broadcasts of changed inodes to clients that have them cached                      |
| 131. | OpenChange cnt/avg/min/max       | Broadcasts of files that have been opened to clients that have<br>the inode cached |
...
| 134. | TokenChangeV4 cnt/avg/min/max    | Broadcasts of changed inodes to clients that have them cached                      |
| 135. | TokenReqAlloc cnt/avg/min/max    | Read or write a file                                                               |
| 136. | TokenChangeSync cnt/avg/min/max  | 3.5 only, not currently used                                                       |
| 137. | TokenChangeV5 cnt/avg/min/max    | 3.5 only, broadcasts of changed inodes to clients that have them<br>cached         |
| 138. | HiPriWr cnt/maxq                 | Number of IOs to the journal and the maximum number queued                         |
|      |                                  | at any one time                                                                    |
| 139. | avg/min/max                      | Time spent waiting for device IO to complete at driver level                       |
| 140. | sysavg/sysmin/sysmax             | Time spent waiting for device IO, including notification from<br>driver            |
...
| 143. | avg/min/max                      | Time spent waiting for device IO to complete at driver level                       |
| 144. | sysavg/sysmin/sysmax             | Time spent waiting for device IO, including notification from<br>driver            |
...
|      |                                  | number queued at any one time                                                      |
| 147. | avg/min/max                      | Time spent waiting for device IO to complete at driver level                       |
| 148. | sysavg/sysmin/sysmax             | Time spent waiting for device IO, including notification from                      |
|      |                                  | driver                                                                             |
