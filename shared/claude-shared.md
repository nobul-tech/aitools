...
- **UCI: Preserve subagent work product** -- When a subagent performs a substantial exploration (multi-file audit, multi-component analysis), write the full findings to the session scratch directory (per `/scratch` skill) -- do not condense them into a stub summary that discards the detail. The SessionEnd hook evaluates for harvesting. Trivial lookups can stay inline.
...
- **USO: Scratch files for complex scripting** -- Never inline long commands in the Bash tool. Write to the session scratch directory (per `/scratch` skill), execute. The SessionEnd hook handles cleanup. Inline only for simple one-liners.
...
- **USO: Simple Bash commands only** -- Never use `$(...)`, backticks, `&&`, `||`, `;`, or glob patterns (`*`, `?`) in destructive commands (`rm`) in Bash tool calls -- these trigger permission prompts that block the user. For commit messages, write to the session scratch directory and use `git commit -F` (per `/scratch` skill). For sequential commands, make separate Bash calls. For cleanup, write a cleanup script and execute it. Pipelines (`|`) are OK.
